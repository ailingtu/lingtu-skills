...
- `packages/video-understand`: video understanding for local files and TikTok/Douyin/YouTube/Instagram URLs — produces a natural-language replication prompt for remixing, tagging, or video breakdown.
...
Use `packages/video-understand` when the user asks to analyze, summarize, tag, break down, or 二创 a single video — local file, uploaded material, or TikTok/Douyin/YouTube/Instagram URL — and wants a replication prompt or content readout.
