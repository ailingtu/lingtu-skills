...
def search_blacklist(unique_ids: list[str]) -> dict[str, Any]:
    ...
    request = urllib_request.Request(
        url,
        data=json.dumps({"uniqueIds": unique_ids}, ensure_ascii=False).encode("utf-8"),
        headers={
    ...
    try:
        with urllib_request.urlopen(request, timeout=60) as http_response:
    ...
    try:
        ...
        raise SystemExit(f"Invalid JSON from {url}: {body[:500]}") from exc
    if not isinstance(response, dict):
        raise SystemExit(f"Invalid blacklist response: {response!r}")
    if response.get("code") != 0:
        message = response.get("message") or "unknown error"
        raise SystemExit(
            f"Blacklist search failed: code={response.get('code')}, "
            f"message={message}, response={json.dumps(response, ensure_ascii=False)}"
        )
    records = extract_records(response)
    return {
        "query": {"uniqueIds": unique_ids},
        "response": response,
        "records": records,
        "missing": missing_ids(unique_ids, records),
    }
...
def print_text(result: dict[str, Any]) -> None:
    ...
    for record in records:
        ...
        print(
            f"{unique_id}: FOUND count={count}, nickname={nickname}, "
            f"region={region}, feedbackAt={feedback_at}, reason={reason}"
        )
...
def main() -> None:
    ...
    search_parser = subparsers.add_parser(
        "search", help="Search blacklist records by TikTok uniqueId."
    )
    search_parser.add_argument(
        "unique_ids",
        nargs="+",
        help="One or more TikTok unique IDs, @handles, or TikTok URLs.",
    )
    search_parser.add_argument(
        "--format",
        choices=("json", "text"),
